...
static void StateChangeNotification (std::string context, LoRaWANPhyEnumeration oldState, LoRaWANPhyEnumeration newState) { NS_LOG_UNCOND (context << " state change at " << Simulator::Now ().GetSeconds () << " from " << oldState << " to " << newState); }
...
int main (int argc, char *argv[])
{
  ...
  Ptr<LoRaWANNetDevice> dev2 = CreateObject<LoRaWANNetDevice> (LORAWAN_DT_END_DEVICE);
  Ptr<LoRaWANNetDevice> dev3 = CreateObject<LoRaWANNetDevice> (LORAWAN_DT_END_DEVICE);
